...
class TemplateMatcher:
    ...
    STRIP_PREFIXES = [
        'needs tuning', 'needs_tuning', 'draft', 'test', 'temp', 'wip',
        'experimental', 'poc', 'updated', '(updated)', 'new'
    ]
    
    STRIP_SUFFIXES = [
        'experimental', 'test', 'draft', 'temp', 'copy', 'backup',
        'v1', 'v2', 'v3', 'old', 'new', 'enhanced', 'advanced'
    ]
    ...
    def key_phrase_match(self, name1: str, name2: str) -> float:
        ...
        key_phrases = [
            ...
            'data exfiltration', 'persistence', 'defense evasion',
            'credential access', 'discovery', 'execution', 'collection',
            'command and control', 'c2', 'initial access', 'impact',
            'mfa', 'multi factor', 'two factor', '2fa',
            'root account', 'admin account', 'service account',
            'api key', 'access key', 'secret key',
            'public access', 'public bucket', 'public share',
            'suspicious', 'anomalous', 'unusual', 'unauthorized',
            'enumeration', 'reconnaissance', 'scanning',
            'policy change', 'configuration change', 'permission change',
            'console login', 'cli access', 'api access'
        ]
