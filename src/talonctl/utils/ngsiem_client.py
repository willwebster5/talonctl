...
class NGSIEMClient:
            ...
            try:
                ...
                if response.get('status_code') != 200:
                    ...
                    return QueryResult(
                        ...
                        error=error_msg,
                        query=query,
                        duration=time.time() - start_exec_time
                    )
                ...
                result = self._poll_for_completion(
                    search_id, config, start_exec_time, query
                )
    ...
    def _poll_for_completion(
        ...
        try:
            while time.time() - start_time < config.timeout:
                status_response = self.client.get_search_status(
                    repository=config.repository,
                    search_id=search_id
                )
    ...
    def test_query_syntax(self, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'valid': False,
                'message': f"Syntax test failed: {str(e)}"
            }
