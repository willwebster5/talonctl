...
try:
    ...
    pass  # Optional dependencies for discovery features
...
class Template:
    ...
    def severity_numeric(self) -> int:
        ...
        severity_map = {"info": 10, "low": 30, "medium": 50, "high": 70, "critical": 90}
...
class TemplateDiscovery:
    ...
    def __init__(self, match_threshold: float = 80.0, dry_run: bool = False, output_format: str = "console"):
        ...
        # Statistics
        self.stats = {"total_templates": 0, "matched": 0, "new": 0, "updated": 0, "skipped": 0, "generated": 0}
    ...
    def _build_fql_filter(
        self, vendor: Optional[List[str]] = None, days_back: Optional[int] = None, _now=None
    ) -> Optional[str]:
        """
    ...
    def fetch_from_api(self, fql_filter: Optional[str] = None) -> List["Template"]:
        ...
        while True:
            ...
            resp = requests.get(
                f"{api_base}/correlation-rules/queries/templates/v1", headers=headers, params=params, timeout=30
            )
            if resp.status_code != 200:
                raise ValueError(f"CRT query API returned {resp.status_code}: {resp.text[:200]}")
        ...
        for i in range(0, len(all_ids), 100):
            ...
            if resp.status_code != 200:
                raise ValueError(f"CRT entities API returned {resp.status_code}: {resp.text[:200]}")
    ...
    def apply_filters(
        ...
        if vendor and "all" not in [v.lower() for v in vendor]:
            ...
            filtered = [
                t for t in filtered if any(v.lower() in [vendor.lower() for vendor in t.vendors] for v in vendor)
            ]
            ...
            if before_vendor > after_vendor and logger.isEnabledFor(logging.DEBUG):
                excluded = [
                    t
                    for t in templates
                    if not any(v.lower() in [vendor.lower() for vendor in t.vendors] for v in vendor)
                ][:5]  # First 5 excluded
        ...
        # MITRE tactic filter
        if mitre_tactic:
            filtered = [t for t in filtered if any(tactic in t.mitre_tactics for tactic in mitre_tactic)]

        # MITRE technique filter
        if mitre_technique:
            filtered = [t for t in filtered if any(technique in t.mitre_techniques for technique in mitre_technique)]
...
"""
        ...
        if self.deployed_state:
            ...
            if deployed_rules:
                ...
                if self.matcher.is_match(deployed_score):
                    self.stats["matched"] += 1
                    logger.debug(
                        f"Template '{template.name}' matched with deployed detection '{deployed_match['name']}' (score: {deployed_score:.1f})"
                    )
                    return None
    ...
    def generate_csv_report(
        self, templates_to_generate: List[Tuple[Template, str]], output_file: Optional[str] = None
    ) -> Optional[str]:
        ...
        for template, action in templates_to_generate:
            ...
            row = {
                ...
                "yaml_file": self.generate_filename(template.name, "(updated)" if action == "updated" else ""),
            }
            rows.append(row)
    ...
    def run(self, **filters):
        ...
        self.console.print(Panel.fit("[bold cyan]🔍 CrowdStrike Template Discovery[/bold cyan]", border_style="cyan"))
        ...
        with Progress(
            ...
            fql_filter = self._build_fql_filter(vendor=filters.get("vendor"), days_back=filters.get("days_back"))
            ...
            try:
                ...
                self.console.print("[yellow]⚠ API unavailable — using local CSV fallback[/yellow]")
                progress.update(task, completed=True)
            ...
            progress.update(task, completed=True)
            self.console.print(
                f"✓ Found [green]{len(self.local_rules)}[/green] rules in {self.rules_dir.relative_to(self.project_root)}"
            )
        ...
        if filters.get("max_new_templates") and len(templates_to_generate) > filters["max_new_templates"]:
            ...
            templates_to_generate.sort(
                key=lambda x: x[0].severity_numeric if hasattr(x[0], "severity_numeric") else 50, reverse=True
            )
            original_count = len(templates_to_generate)
            templates_to_generate = templates_to_generate[: filters["max_new_templates"]]
            self.console.print(
                f"[yellow]Limited to {filters['max_new_templates']} templates (was {original_count})[/yellow]"
            )
        ...
        if filters.get("priority_filter"):
            ...
            if allowed_severities:
                ...
                self.console.print(
                    f"[yellow]Priority filter applied: {len(templates_to_generate)} of {original_count} templates match[/yellow]"
                )
        ...
        if filters.get("exclude_experimental"):
            ...
            templates_to_generate = [
                (template, action)
                for template, action in templates_to_generate
                if not any(keyword in template.name.lower() for keyword in ["experimental", "beta", "test", "poc"])
            ]
            self.console.print(
                f"[yellow]Excluded experimental: {len(templates_to_generate)} of {original_count} templates remaining[/yellow]"
            )
        ...
        if filters.get("output_format") == "json":
            ...
            self.console.print(
                f"  - Updated templates generated: {sum(1 for _, a in templates_to_generate if a == 'updated')}"
            )
            self.console.print(f"  - Total files created: {self.stats['generated']}")

            if not self.dry_run:
                output_dir = (
                    "templates_review" if filters.get("auto_review") else self.review_dir.relative_to(self.project_root)
                )
