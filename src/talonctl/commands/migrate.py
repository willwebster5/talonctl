...
@click.pass_context
def migrate(ctx, state_file, write, templates_only, state_only, fmt, output):
    ...
    if do_state:
        sm = StateManager(state_file_path=get_state_file_path(state_file))
        discovery = TemplateDiscovery()
        index = build_template_index(discovery.discover_all())
        resources = sm.export_to_dict().get("resources", {})
        report.state = reconcile_state(resources, index)
        if write and report.state.rekeyed:
