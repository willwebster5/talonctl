...
class TestProviderAdapter:
    ...
    def test_plan_detection_changes_create(self, adapter):
        ...
        adapter.detection_provider.plan_create = Mock(
            return_value=Mock(action=ResourceAction.CREATE, resource_name="new_rule", resource_id=None)
        )
    ...
    def test_plan_detection_changes_update(self, adapter):
        ...
        # Mock provider methods
        adapter.detection_provider.validate_template = Mock(return_value=[])
        adapter.detection_provider.fetch_remote_state = Mock(
            return_value={"id": "rule123", "name": "Existing Rule", "description": "Old description"}
        )
        adapter.detection_provider.plan_update = Mock(
            return_value=Mock(action=ResourceAction.UPDATE, resource_name="existing_rule", resource_id="rule123")
        )
    ...
    def test_plan_detection_changes_delete(self, adapter):
        ...
        # Mock provider methods
        adapter.detection_provider.plan_delete = Mock(
            return_value=Mock(action=ResourceAction.DELETE, resource_name="old_rule", resource_id="rule123")
        )
    ...
    def test_plan_resource_changes_create(self, adapter):
        ...
        templates = {
            "new_rule": {"name": "New Rule", "description": "Test", "severity": 50, "_template_path": "rules/new.yaml"}
        }
    ...
    def test_apply_resource_change_create_updates_state(self, adapter):
        ...
        adapter.detection_provider.apply_create = Mock(
            return_value={"rule_id": "abc123", "created_at": "2025-01-01T00:00:00Z"}
        )
