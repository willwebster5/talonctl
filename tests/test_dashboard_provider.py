    ...
    return {
        ...
        'sections': {
            'section-1': {
                ...
                'order': 0,
                'title': 'Section One',
                'widgetIds': ['widget-aaa']
            }
        },
        'widgets': {
            'widget-aaa': {
                'x': 0, 'y': 0, 'height': 4, 'width': 12,
                'title': 'Test Widget',
                'type': 'query',
...
class TestValidateTemplate:
    ...
    def test_note_widget_no_querystring_ok(self, provider, valid_template):
        valid_template['widgets']['widget-aaa'] = {
            'x': 0, 'y': 0, 'height': 2, 'width': 12,
            'title': 'Note', 'type': 'note', 'text': 'Hello'
        }
        errors = provider.validate_template(valid_template)
        assert errors == []
    ...
    def test_parameter_panel_no_querystring_ok(self, provider, valid_template):
        valid_template['widgets']['widget-aaa'] = {
            'x': 0, 'y': 0, 'height': 2, 'width': 12,
            'title': 'Filters', 'type': 'parameterPanel',
            'parameterIds': ['param1']
        }
        errors = provider.validate_template(valid_template)
        assert errors == []
...
class TestExtractDependencies:
    def test_extracts_saved_search_refs(self, provider):
        template = {
            'widgets': {
                'w1': {
                    'type': 'query',
                    'queryString': '| $identity_enrich_from_email() | $score_geo_risk()'
                }
            },
            'parameters': {},
            'sections': {}
    ...
    def test_extracts_lookup_file_refs(self, provider):
        template = {
            'widgets': {
                'w1': {
                    'type': 'query',
                    'queryString': '| match(file="cato-users.csv", field=ComputerName, column=x)'
                }
            },
            'parameters': {},
            'sections': {}
    ...
    def test_extracts_from_parameter_queries(self, provider):
        template = {
            ...
            'parameters': {
                'dept': {
                    'type': 'query',
                    'query': '| $identity_enrich_from_email() | groupBy([id.department])'
                }
            },
            'sections': {}
        }
    ...
    def test_no_deps_returns_empty(self, provider):
        template = {
            'widgets': {
                'w1': {'type': 'query', 'queryString': 'count()'}
            },
            'parameters': {},
            'sections': {}
        }
...
class TestCreateResource:
    ...
    def test_failure_raises(self, provider, mock_falcon, valid_template):
        mock_falcon.command.return_value = {
            'status_code': 500,
            'body': {'errors': [{'message': 'Create failed'}]}
        }
...
class TestFetchAllRemoteDashboards:
    def test_returns_dict_keyed_by_name(self, provider, mock_falcon):
        mock_falcon.command.return_value = {
            'status_code': 200,
            'body': {'resources': [
                {'id': 'dash-1', 'name': 'Dashboard One'},
                {'id': 'dash-2', 'name': 'Dashboard Two'}
            ]}
        }
    ...
    def test_caches_result(self, provider, mock_falcon):
        mock_falcon.command.return_value = {
            'status_code': 200,
            'body': {'resources': [{'id': 'dash-1', 'name': 'D1'}]}
        }
